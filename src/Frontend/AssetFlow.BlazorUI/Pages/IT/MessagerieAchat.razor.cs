// ============================================================
// AssetFlow.BlazorUI / Pages / IT / MessagerieAchat.razor.cs
// Messagerie IT ↔ Agent Achat — miroir de Messagerie.razor.cs
// ============================================================
...
namespace AssetFlow.BlazorUI.Pages.IT
{
    ...
    public partial class MessagerieAchat : IAsyncDisposable
    {
        ...
        private string SearchAchat { get; set; } = string.Empty;
        private string NewMessage  { get; set; } = string.Empty;

        private List<AchatUserConvDto>  AchatUsers  { get; set; } = new();
        private AchatUserConvDto?       SelectedUser { get; set; }
        ...
        private List<AchatUserConvDto> AchatUsersFiltres =>
            (string.IsNullOrWhiteSpace(SearchAchat)
                ? AchatUsers
                : AchatUsers.Where(u => u.FullName.Contains(SearchAchat, StringComparison.OrdinalIgnoreCase)))
            ...
            try
            {
                ...
                _hub.On<ChatMessageDto>("ReceiveMessage", async msg =>
                {
                    await InvokeAsync(async () =>
                    {
                        ...
                        {
                            if (!CurrentMessages_HasOptimistic(msg))
                            {
                ...
                _hub.On<int, bool>("UserOnlineStatus", async (userId, isOnline) =>
                {
                    await InvokeAsync(() =>
                    {
                        var u = AchatUsers.FirstOrDefault(x => x.Id == userId);
        ...
        // ── Chargement des agents achat ──────────────────────────
        private async Task LoadAchatUsersAsync()
        {
            LoadingUsers = true;
            StateHasChanged();
            ...
            try
            {
                // Endpoint qui retourne les utilisateurs du rôle EquipeAchat
                var users = await Http.GetFromJsonAsync<List<AchatUserSimpleDto>>("api/users/achat") ?? new();
                AchatUsers = users.Select(u => new AchatUserConvDto
                {
                    Id       = u.Id,
                    FullName = u.FullName,
                    Initials = u.Initials,
                ...
                // Récupérer les résumés de conversation
                var summaries = await MsgSvc.GetConversationsAsync(CurrentUserId);
                foreach (var s in summaries)
                {
                    var u = AchatUsers.FirstOrDefault(x => x.Id == s.OtherUserId);
                    if (u != null)
                    {
        ...
        private async Task SelectUser(AchatUserConvDto user)
        {
            SelectedUser      = user;
        ...
        {
            var otherId = msg.SenderId == CurrentUserId ? msg.ReceiverId : msg.SenderId;
            var u = AchatUsers.FirstOrDefault(x => x.Id == otherId);
            if (u == null) return;
        ...
        private bool CurrentMessages_HasOptimistic(ChatMessageDto msg) =>
        ...
        {
            await Task.Delay(50);
            try { await JS.InvokeVoidAsync("scrollToBottom", "miaMessagesZone"); } catch { }
        }
    ...
    // DTO simple reçu de l'API
    public class AchatUserSimpleDto
    {
        public int    Id       { get; set; }
        public string FullName { get; set; } = string.Empty;
        public string Initials { get; set; } = string.Empty;
